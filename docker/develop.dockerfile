...
FROM netologygroup/gstreamer:1.16-buster as gstreamer
...
COPY --from=gstreamer "/usr/lib/gstreamer-1.0" "/usr/lib/gstreamer-1.0"
# TODO: create symlinks, docker just copies files
COPY --from=gstreamer "/usr/lib/libgst*" "/usr/lib/"
